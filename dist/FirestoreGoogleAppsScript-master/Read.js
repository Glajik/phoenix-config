...
function getDocumentIds_(path, request) {
  var documents = query_(path, request).select().execute();
  var ids = documents.map(function (doc) {
    var ref = doc.name.match(regexPath_)[1]; // Gets the doc name field and extracts the relative path
...
function query_(path, request) {
  var grouped = getCollectionFromPath_(path);
  var callback = function callback(query) {
    // Send request to innermost document with given query
    var responseObj = request.post(grouped[0] + ':runQuery', {
      structuredQuery: query
    });

    // Filter out results without documents and unwrap document fields
    var documents = responseObj.reduce(function (docs, fireDoc) {
      if (fireDoc.document) {
