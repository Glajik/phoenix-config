 ...
 */
var FirestoreQuery_ = function FirestoreQuery_(from, callback) {
  var this_ = this;
  ...
  };var fieldRef = function fieldRef(field) {
    return { fieldPath: field };
  };
  var filter = function filter(field, operator, value) {
  ...
  this.select = function (field) {
    ...
    if (!field || !field.trim()) {
      // Catch undefined or blank strings
      field = '__name__';
    }
  ...
  this.orderBy = function (field, dir) {
    ...
    var isDesc = dir && (dir.substr(0, 3).toUpperCase() === 'DEC' || dir.substr(0, 4).toUpperCase() === 'DESC');
