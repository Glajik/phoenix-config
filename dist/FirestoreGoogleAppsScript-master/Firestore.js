 ...
 * @return {object} an authenticated interface with a Firestore project
 */
var Firestore = function Firestore(email, key, projectId) {
  /**
   * The authentication token used for accessing Firestore.
   */
  var authToken = getAuthToken_(email, key, 'https://oauth2.googleapis.com/token');
  var baseUrl = 'https://firestore.googleapis.com/v1beta1/projects/' + projectId + '/databases/(default)/documents/';
  ...
  this.getDocument = function (path) {
    var request = new FirestoreRequest_(baseUrl, authToken);
